...
namespace WowApp
{
    public class AppWrapper
    {
        ...
        public void SendKey(string appExeName, char key)
        {
            try
            {
                lock (Processes)
                {
                    foreach (Process process in Process.GetProcessesByName(appExeName).ToList())
                    {
                        if (process.MainWindowHandle != IntPtr.Zero)
                        {
                            Trace.WriteLine($"ProcessName={process.ProcessName} - {process.MainWindowTitle}");
        ...
        public void SendKey(string appExeName, Key key)
        {
            try
            {
                lock (Processes)
