...
namespace App.Core.Infra.Extensions
{
    public class SqlTableCreator
    {
        ...
        public static string GetCreateSQL(string tableName, DataTable schema, int[] primaryKeys)
        {
            ...
            foreach (DataRow dataRow in schema.Rows)
            {
                    ...
                    && dataRow["IsHidden"] != DBNull.Value && (bool)dataRow["IsHidden"]))
                {
                    ...
                    if (schema.Columns.Contains("AllowDBNull") && dataRow["AllowDBNull"] != DBNull.Value && !(bool)dataRow["AllowDBNull"])
                        sql += " NOT NULL";
                    sql += ",\n";
                }
            }
        ...
        public static string[] GetPrimaryKeys(DataTable schema)
        {
            ...
            foreach (DataRow dataRow in schema.Rows)
            {
                if (schema.Columns.Contains("IsKey") && dataRow["IsKey"] != DBNull.Value && (bool)dataRow["IsKey"])
                    keys.Add(dataRow["ColumnName"].ToString());
            }

            return keys.ToArray();
        }

        public static int[] GetPrimaryKeysIndexes(DataTable schema)
        {
